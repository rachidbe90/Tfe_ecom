    ...
    @if(\Gloudemans\Shoppingcart\Facades\Cart::instance('wishlist')->count()>0)
        @foreach(\Gloudemans\Shoppingcart\Facades\Cart::instance('wishlist')->content() as $item)
            <tr>
                ...
                <td>
                    <img src="{{$item->model->photo}}" alt="Product" class="img-fluid" style="max-width: 100px;">
                </td>
                <td>
                ...
                <td>
                    <a href="javascript:void(0);" data-id="{{$item->rowId}}" class="wishlist_delete btn btn-danger btn-sm">Delete</a>
                </td>
            </tr>
        @endforeach
    @else
